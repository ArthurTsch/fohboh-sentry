# CAAR 92 - Court Admissible

Expected outcome:
- Clean upload path
- Full evidence package present
- Trust Score should land in the pass band
- CAAR should be eligible for court-admissible output and claim-pack generation

Use these files on one location configured for:
- `M01`: `Heartland`
- `M02`: `DoorDash`

Upload set:
- `FohBoh_Test_M01_Heartland_Processor_Statement.csv`
- `FohBoh_Test_M01_Heartland_POS_Export.csv`
- `FohBoh_Test_M02_DoorDash_Settlement.csv`
- `FohBoh_Test_M02_DoorDash_POS_Summary.csv`
- `FohBoh_Test_M02_DoorDash_Agreement.pdf`
- `FohBoh_Test_M02_Bank_Statement.pdf`

Notes:
- This is the clean reference pack.
- Use this first when verifying the happy-path certification and CAAR flow.
